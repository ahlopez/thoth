...
implements HasValue<E, T>
{
	...
	private TreeGrid<T> buildSelector(boolean showId)
	{
		...
		if (showId)
		{
			tGrid.addHierarchyColumn(T::getName).setHeader("Nombre").setFlexGrow(75);
			tGrid.addColumn(T::formatCode).setHeader("Id").setFlexGrow(25);
		}else 
		{
	...
	private Grid<T> buildSearchGrid(TreeGrid<T> tGrid, boolean showId)
	{
		...
		if (showId)
		{
			sGrid.addColumn(T::getName).setHeader("Nombre").setFlexGrow(75);
			sGrid.addColumn(T::formatCode).setHeader("Id").setFlexGrow(25);
		}else 
		{
