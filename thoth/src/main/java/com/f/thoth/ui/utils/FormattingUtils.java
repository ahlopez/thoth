...
public class FormattingUtils {
    ...
    */
   public static final TemporalField WEEK_OF_YEAR_FIELD = WeekFields.of(Constant.APP_LOCALE).weekOfWeekBasedYear();
   ...
   public static String getFullMonthName(LocalDate date) {
      return date.getMonth().getDisplayName(TextStyle.FULL, Constant.APP_LOCALE);
   }

   public static String formatAsCurrency(int valueInCents) {
      return NumberFormat.getCurrencyInstance(Constant.APP_LOCALE).format(BigDecimal.valueOf(valueInCents, 2));
   }

   public static DecimalFormat getUiPriceFormatter() {
      DecimalFormat formatter = new DecimalFormat("#" + DECIMAL_ZERO,
            DecimalFormatSymbols.getInstance(Constant.APP_LOCALE));
